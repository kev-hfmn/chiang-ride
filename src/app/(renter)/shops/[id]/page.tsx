...
export default async function ShopDetailPage({
  ...
  const [shop, scooters, reviews, ratingStats] = await Promise.all([
    getShop(id),
    getScooters(id), // Now includes availability data
    getShopReviews(id),
  ...
  return (
    <div className="space-y-6 pb-20">
      ...
      <div>
        ...
        {/* Shop Image */}
        <div className="w-full h-64 sm:h-80 bg-gray-100 rounded-2xl relative overflow-hidden">
          {shop.image_url ? (
            <OptimizedImage
              src={shop.image_url}
              alt={shop.name}
          ...
          {/* Verified Badge */}
          {shop.is_verified && (
            <div className="absolute top-4 right-4 bg-white/95 backdrop-blur-sm px-3 py-1.5 rounded-full text-xs font-semibold text-orange-700 flex items-center gap-1.5 shadow-sm">
            ...
            >
              <Link href={`/scooters/${scooter.id}`}>
                ...
                <div className="p-4 flex gap-4">
                  {/* Image Placeholder */}
                  <div className="w-24 h-24 shrink-0 bg-gray-100 rounded-2xl relative overflow-hidden">
                    <ScooterImage
                      brand={scooter.brand}
                      model={scooter.model}
