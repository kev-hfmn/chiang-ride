...
export default function BookingForm({ scooter, shopId }: BookingFormProps) {
    ...
    return (
        <form action={createBookingRequestAction} onSubmit={() => setIsSubmitting(true)} className="bg-gray-50 rounded-xl p-4 space-y-4">
            ...
            <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                    <label className="block text-xs font-bold text-gray-500 uppercase mb-1">Dropoff</label>
                    <input 
                        type="date" 
                        name="end_date"
                        min={startDate || today}
            ...
            {totals && (
                <div className="flex justify-between items-center py-2 border-t border-gray-200 mt-2">
                    <span className="text-sm font-bold text-gray-500">{totals.days} Days</span>
                    <span className="text-lg font-extrabold text-gray-900">{totals.price}฿ <span className="text-xs font-normal text-gray-400">Total</span></span>
                </div>
            )}
